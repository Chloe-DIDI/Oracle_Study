...
CREATE OR REPLACE TRIGGER TRG_TEST2_DELETE
        BEFORE                  --> ★
        DELETE ON TBL_TEST2     --> TEST2에 DELETE 를 수행하겠다
        FOR EACH ROW            --> ★ ROW 는 행
BEGIN
    DELETE
    FROM TBL_TEST3
    WHERE CODE=:OLD.CODE;
...
-- TBL_상품, TBL_입고, TBL_출고 → 이 테이블 활용 할 것
...
--    서브 프로그램(PROCEDURE, FUNCTION 등)을 논리적으로 묶어 놓은 것으로
...
-- 2. 몸체부 작성              -----------▶위에꺼랑 똑같아야된다 _이거까지!!
CREATE OR REPLACE PACKAGE BODY INSA_PACK
IS              ----------------------------------------- 여기 사이에 176행 - 190행 붙여넣기
    FUNCTION FN_GENDER(V_SSN VARCHAR2)
    RETURN VARCHAR2
    IS 
        V_RESULT VARCHAR2(20);
