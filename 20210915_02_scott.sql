...
★★★★ⓐⓑⓒ 이거 20210915_01_scott(plsql) 랑 왔다갔다 하면서 푸는 것!★★★★
20210915_01_scott(plsql) 에서 FN_GENDER, FN_FOW 만든게 여기로 호출 된 것!
...
EXEC PRC_STUDNETS_INSERT('batman', 'java006$', '송해덕', '010-2222-2222', '경기도 고양시');
...
ENG             NUMBER(3)    
MAT             NUMBER(3)    
TOT             NUMBER(3)       --> 추가된거 CHECK~!
AVG             NUMBER(4,1)     --> CHECK
GRADE           CHAR(1)         --> CHECK
*/

-- ★★★★  ⒜   ★★★★
--○ 생성한 프로시저 가 제대로 작동하는지의 여부 확인
-- → 프로시저 호출
EXEC PRC_SUNGJUK_INSERT(1, '김희진', 90, 80, 70);
--==>> PL/SQL 프로시저가 성공적으로 완료되었습니다.

--잘들어갔는지 확인
SELECT *
FROM TBL_SUNGJUK;
--==>> 1	김희진	90	80	70	240	80	B
...
FROM TBL_SUNGJUK;
--==>>
/*          숫자 뿐만아니라    등급도 알아서 바껴서 나온다.
1	김희진	55	66	77	198	66	D
2	김소연	100	100	100	300	100	A
*/
...
--==>> 패스워드 맞지 않음
-->   근데 .... ?????  이렇게  뜬다 --> PL/SQL 프로시저가 성공적으로 완료되었습니다. ---> 틀렸기때문에 데이터 변화없음,,
...
EXEC PRC_INSA_INSERT(V_NUM, 이다영, '951027-2234567',SYSDATE, '서울', '010-4113-2353', '영업부', '대리', 10000000, 2000000);
--==>> PL/SQL 프로시저가 성공적으로 완료되었습니다.
