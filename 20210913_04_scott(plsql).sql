...
--○ SCOTT.TBL_INSA 테이블의 여러 명의 데이터를 여러 개를 출력
--   (반복문 활용)
...
--   (반복문 활용 -> 실무에서는 커서를 사용함. 현재는 사용할 수 없으므로 반복문 활용!)
DECLARE
...
BEGIN
    LOOP
        ...
        DBMS_OUTPUT.PUT_LINE(VINSA.NAME || ' - ' || VINSA.TEL || ' - '
                                || VINSA.BUSEO);
        EXIT WHEN VNUM >= 1060; -- or VNUM > 1059
