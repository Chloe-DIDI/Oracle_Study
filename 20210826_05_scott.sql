...
--○ TBL_EXAMPLE1 와 TBL_EXAMPLE2 테이블이 
--   어떤 테이블 스펭이스에 저장되어 있는지 조회
SELECT TABLE_NAME, TABLESPACE_NAME
FROM USER_TABLES;
--==>>
/*
...
--==>>
/*
사원번호사원이름  직종  매니저이름   입사일     급여   수당   부서번호
...
/*
등급  최저급여 최고급여
1   	700	    1200            -- 급여 700~1200받는 사람은 1등급
2	    1201	1400
3   	1401	2000
4   	2001	3000
5	    3001	9999
...
/*
2. 문자형 CHAR(n)          - 고정형 크기
          CHAR(10)         ← '강의실' //'강의실'은 6byte지만 CHAR을 10byte로 설정
                                       //해두었기때문에 10 byte를 소모
          CHAR(10)         ← '졸리서승균' 10 byte
          CHAR(10)         ← 'OH집중서승균' 12 byte // 10byte 초과해서 담을 수 없다. 
          
          VARCHAR2          - 가변형 크기
          VARCHAR2(10)      ← '강의실'          6byte라  6byte 만 차지    
          VARCHAR2(10)      ← '잠깬서승균'      10 byte
          CHAR(10)          ← 'OH잠깬서승균'  12 byte // 10byte 초과해서 담을 수 없다. 
...
FROM EMP
WHERE DEPTNO = 20;     
--WHERE DEPTNO == 20;     -- X  오라클에서 == 같다라는 표시아니고 오라클은 = 이거
--WHERE 부서번호가 20번;--==>>
/*
...
--※ 테이블을 조회하는과정에서 각 컬럼에 별칭(ALIAS)을 부여할 수 있다.
